...
socket.on('joinedRoomSuccessfully', ({ state, question, options }) => {
    ...
    if (state === 'waiting') waitSection.classList.remove('hidden');
    else if (state === 'voting') showVotingScreen(question, options);
    else { clientStatusMessage.innerText = "This poll has concluded."; clientResultsSection.classList.remove('hidden'); }
});
...
function showVotingScreen(question, options) {
    voteQuestion.innerText = question; voteOptionsBox.innerHTML = '';
    options.forEach(option => {
        const btn = document.createElement('button'); btn.className = 'btn-option'; btn.innerText = option;
        btn.addEventListener('click', () => { socket.emit('submitVote', option); disableVotingButtons(); });
...
function disableVotingButtons() {
    ...
    setTimeout(() => { voteSection.classList.add('hidden'); clientResultsSection.classList.remove('hidden'); }, 1200);
}
socket.on('liveResultsUpdate', ({ options, totalVotes }) => { renderBars(clientResultsContainer, options, totalVotes); });
